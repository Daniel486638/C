...
int main(int ArgLength, char **Args) {
	...
	printf("Program was created by Daniel486638.\nSend I or O and then send your key for the hot key for in or out respectively (for example, if you send I and then E, in block will be activated by CTRL + E).\n");//ShowWindow(Window, SW_HIDE);
	DWORD PID;
	//sscanf(Args[1], "%d", &PID);
	...
	while (1) {
		if (GetAsyncKeyState(VK_CONTROL) && GetAsyncKeyState(letter)) {
			...
			strcpy(command, "netsh advfirewall firewall delete rule name=\"!BlockI ");
			strcat(command, str4num);
			strcat(command, "\"");
			system(command);
...
void* second(void*) {
	...
	while (1) {
		if (GetAsyncKeyState(VK_CONTROL) && GetAsyncKeyState(letter2)) {
			...
			strcpy(command, "netsh advfirewall firewall delete rule name=\"!BlockO ");
			strcat(command, str4num);
			strcat(command, "\"");
			system(command);
