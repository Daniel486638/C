...
int main(int ArgLength, char **Args) {
	...
	while (1) {
		...
		if (press && !started && GetAsyncKeyState('G') < 0) {
			keyPress(0x20);
			started = true;
			while (GetAsyncKeyState('G') < 0 || (GetAsyncKeyState(letter2) < 0 && GetAsyncKeyState(letter) < 0)) {Sleep(1);};
		} else if (press && started && GetAsyncKeyState('G') >= 0) {
			keyRelease(0x20);
			started = false;
		};
		Sleep(1);
